...
void Character::pickItem() {
    item_num++;
    cout << " 아이템 1개를 획득! 남은 아이템 :  " << item_num << endl;
    saveToFile();
}
...
void Character::useItem() {
    if (item_num > 0) {
        item_num--;
        cout << " 아이템 1개를 사용! 남은 아이템 :  " << item_num << endl;
        saveToFile();
    }
    ...
    if (file.is_open()) {
        ...
        while (getline(file, line)) {
            ...
            if (name_in_file == targetName) {
                cout << "\n" << name << " 이(가)  " << targetName << "을(를) 공격하였습니다!" << endl;
