...
예시 0. 저번 시간에 배운 파일 입출력을 적용해서 파일에 캐릭터들의 정보 저장하기
예시 1. 발생 가능한 에러 처리하기(ex.아이템이 없으면 사용 못하게, 겹치는 이름 오류 출력하고 다시 입력받기)
예시 2. 새로운 필드 및 메소드 추가(ex.체력, 공격하기)
예시 3. 객체끼리 상호작용 시키기(ex.이름 지목해서 공격하기, 체력 -10) */
