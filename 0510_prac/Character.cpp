    ...
    if (file.is_open()) {
        ...
        while (getline(file, line_name)) {
            ...
            if (name_in_file == name) {
                ...
                while (!validName) {
                    cout << "새로운 이름을 입력해 주세요 (변경 중단 : exit) :  ";
                    cin >> newName;

                    // 변경될 이름이 현재 이름과 같은지 확인
                    if (newName == name) {
                        cout << "현재 이름과 동일합니다. 다른 이름을 입력하세요.\n" << endl;
                        continue;
                    }
                    ...
                    while (getline(file, line_name)) {
                        stringstream ss_check(line_name);
                        string existingName;
                        ss_check >> existingName;
                        if (existingName == newName) {
                            cout << "이미 존재하는 이름입니다. 다른 이름을 입력하세요.\n" << endl;
                            nameExists = true;
                            break;
                        }
                    }
                ...
                file.clear(); // eof 비트를 초기화하여 파일의 끝에 도달하더라도 eof 비트가 설정되지 않도록 함
                file.seekg(0, ios::beg); // 파일의 시작으로 이동
